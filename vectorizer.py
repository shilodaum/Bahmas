...
def get_list_of_texts():
    ...
    for file in txt_files:
        with open(os.path.join(tiuli_titles_folder_path, file), 'r', encoding="utf8") as f:
            corpus = f.read()
    ...
    for file in txt_files:
        with open(os.path.join(maslulim_israel_titles_folder_path, file), 'r', encoding="utf8") as f:
            corpus = f.read()
            tokens = tokenization(corpus)
            connected_tokens = " ".join(tokens)
            all_txt_files.append(connected_tokens)
...
def main():
    X_transformed = download_df_csv()
    print(get_features('texts_vectors.csv'))
    clustering(X_transformed, 'texts_vectors.csv')
